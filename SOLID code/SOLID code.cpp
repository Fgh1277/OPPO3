    ...
    void loadFromFile(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            if (type == "tree") {
                int age;
                iss >> age;
                addPlant(std::make_shared<Tree>(name, age));
            }
            else if (type == "shrub") {
                std::string flowering_month;
                iss >> flowering_month;
                addPlant(std::make_shared<Shrub>(name, flowering_month));
            }
            else if (type == "cactus") {
                double spine_length;
                iss >> spine_length;
                addPlant(std::make_shared<Cactus>(name, spine_length));
            }
            else {
                std::cout << "Неизвестный тип растения: " << type << std::endl;
            }
        }
